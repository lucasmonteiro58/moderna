...
hideVideoControlsOnLoad:false,clickToPlayPause:true,iPadUseNativeControls:false,iPhoneUseNativeControls:false,AndroidUseNativeControls:false,features:["playpause","current","progress","duration","tracks","volume","fullscreen","options"],isVideo:true,enableKeyboard:false,pauseOtherPlayers:false,keyActions:[/*{keys:[32,179],action:function(a,b){b.paused||b.ended?a.play():a.pause()}},{keys:[38],action:function(a,b){b.setVolume(Math.min(b.volume+0.1,1))}},{keys:[40],action:function(a,b){b.setVolume(Math.max(b.volume-
...
(function(f){f.extend(mejs.MepDefaults,{playpauseText:mejs.i18n.t("PAUSAR VÍDEO")});f.extend(MediaElementPlayer.prototype,{buildplaypause:function(a,b,c,e){var d=f('<div class="mejs-button mejs-playpause-button mejs-play " ><button type="button" aria-controls="'+this.id+'" title="'+this.options.playpauseText+'" class="tabindex"></button></div>').appendTo(b).click(function(g){g.preventDefault(); if(e.paused){ e.play(); f('.mejs-playpause-button button').attr('title','PAUSAR VÍDEO'); }else{ e.pause(); f('.mejs-playpause-button button').attr('title','REPRODUZIR VÍDEO'); } f('.mejs-playpause-button button')[0].focus(); return false});e.addEventListener("play",function(){d.removeClass("mejs-play").addClass("mejs-pause")},
false);e.addEventListener("playing",function(){d.removeClass("mejs-play").addClass("mejs-pause")},false);e.addEventListener("pause",function(){d.removeClass("mejs-pause").addClass("mejs-play")},false);e.addEventListener("paused",function(){d.removeClass("mejs-pause").addClass("mejs-play")},false)}})})(mejs.$);
(function(f){f.extend(MediaElementPlayer.prototype,{buildoptions:function(a,b,c,e){ f('<div class="mejs-options"> <button type="button" aria-controls="'+this.id+'" title="VELOCIDADE NORMAL" id="btnPlrSpeed" class="mejs-button tabindex" data-velocidade="1" data-controller="no_speed">1.0 x</button> <button type="button" aria-controls="'+this.id+'" title="LEGENDA DESATIVADA" id="btnPlrCC" class="mejs-button tabindex" data-show="false" data-controller="no_close_caption"></button> <button type="button" aria-controls="'+this.id+'" title="OPÇÕES DE ACESSIBILIDADE" id="btnPlrOpt" class="mejs-button tabindex" data-show="false"></button> </div>').appendTo(b) }})})(mejs.$);
...
this,g=b.find(".mejs-time-total");c=b.find(".mejs-time-loaded");var k=b.find(".mejs-time-current"),j=b.find(".mejs-time-handle"),m=b.find(".mejs-time-float"),q=b.find(".mejs-time-float-current"),p=function(h){das=Functions.getOffset(h),h=das.x;var l=g.offset(),r=g.outerWidth(true),n=0,o=n=0;if(e.duration){/*if(h<l.left)h=l.left;else if(h>r+l.left)h=r+l.left;*/o=h/*-l.left*/;n=o/r;n=n<=0.02?0:n*e.duration;t&&n!==e.currentTime&&e.setCurrentTime(n);if(!mejs.MediaFeatures.hasTouch){m.css("left",o);q.html(mejs.Utility.secondsToTimeCode(n));
...
(function(f){f.extend(mejs.MepDefaults,{muteText:mejs.i18n.t("SOM"),hideVolumeOnTouchDevices:true,audioVolume:"horizontal",videoVolume:"vertical"});f.extend(MediaElementPlayer.prototype,{buildvolume:function(a,b,c,e){if(!(mejs.MediaFeatures.hasTouch&&this.options.hideVolumeOnTouchDevices)){var d=this,g=d.isVideo?d.options.videoVolume:d.options.audioVolume,k=g=="horizontal"?f('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button" aria-controls="'+d.id+'" title="'+d.options.muteText+
...
(function(f){f.extend(mejs.MepDefaults,{startLanguage:"",tracksText:mejs.i18n.t("Captions/Subtitles"),hideCaptionsButtonWhenEmpty:true,toggleCaptionsButtonWhenOnlyOne:false,slidesSelector:""});f.extend(MediaElementPlayer.prototype,{hasChapters:false,buildtracks:function(a,b,c,e){if(a.tracks.length!=0){var d;if(this.domNode.textTracks)for(d=this.domNode.textTracks.length-1;d>=0;d--)this.domNode.textTracks[d].mode="hidden";a.chapters=f('<div class="mejs-chapters mejs-layer" aria-hidden="true" tabindex="-1"></div>').prependTo(c).hide();a.captions=
f('<div class="mejs-captions-layer mejs-layer" aria-hidden="true" tabindex="-1"><div class="mejs-captions-position mejs-captions-position-hover" aria-hidden="true" tabindex="-1"><span class="mejs-captions-text" aria-hidden="true" tabindex="-1"></span></div></div>').prependTo(c).hide();a.captionsText=a.captions.find(".mejs-captions-text");a.captionsButton=f('<div class="mejs-button mejs-captions-button"><button type="button" aria-controls="'+this.id+'" title="'+this.options.tracksText+'" aria-label="'+this.options.tracksText+'"></button><div class="mejs-captions-selector"><ul><li><input type="radio" name="'+
...
"undefined"){var a,b=this.selectedTrack;if(b!=null&&b.isLoaded)for(a=0;a<b.entries.times.length;a++)if(this.media.currentTime>=b.entries.times[a].start&&this.media.currentTime<=b.entries.times[a].stop){ if(b.entries.times[a].settings.indexOf('line:') != -1){ var value = b.entries.times[a].settings.split('line:'); value = value[1].split('%'); var posicao = 525 - (( 600 * parseInt(value[0]) ) / 100); f('.mejs-captions-position').css('bottom',posicao); }else{  f('.mejs-captions-position').removeAttr('style'); } this.captionsText.html(b.entries.text[a].replace('\n','<br/>'));this.captions.show().height(0);return}this.captions.hide()}},setupSlides:function(a){this.slides=a;this.slides.entries.imgs=[this.slides.entries.text.length];this.showSlide(0)},showSlide:function(a){if(!(typeof this.tracks=="undefined"||typeof this.slidesContainer==
